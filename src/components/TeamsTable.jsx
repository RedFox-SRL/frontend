...
export default function TeamsTable() {
    ...
    const filteredTeams = teams.filter(team =>
        ...
        (team.long_name && team.long_name.toLowerCase().includes(searchTerm.toLowerCase()))
    );
    ...
    return (
        <div className="w-full max-w-4xl mx-auto bg-white shadow-xl rounded-lg overflow-hidden">
            <div className="p-6 bg-gradient-to-r from-purple-600 to-purple-800">
                <h2 className="text-2xl font-bold text-white mb-4">Grupo empresas registrados</h2>
            </div>
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow className="bg-purple-100">
                                    <TableHead className="w-1/3 sm:w-1/2 font-bold text-purple-800 px-4 py-3">Nombre corto</TableHead>
                                    <TableHead className="w-2/3 sm:w-1/2 font-bold text-purple-800 px-4 py-3">Nombre largo</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {isLoading ? (
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={2} className="text-center text-purple-600 py-4">
                                    ...
                                    filteredTeams.map((team, index) => (
                                        ...
                                        >
                                            <TableCell className="font-medium text-purple-700 px-4 py-3 break-words">{team.short_name}</TableCell>
                                            <TableCell className="text-purple-600 px-4 py-3 break-words">{team.long_name || '-'}</TableCell>
                                        </TableRow>
                                    ))
                                )}
                            </TableBody>
