    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {attachments.map((attachment, index) => (
                <div key={index} className="relative bg-gray-100 p-4 rounded-lg flex items-center space-x-3">
                    <div className="flex-shrink-0">
                        {attachment.type === 'file' ? (
                            ...
                            <img src={attachment.icon} alt={attachment.name} className="w-12 h-12"/>
                        ) : attachment.type === 'youtube' && (
                            <img src={attachment.thumbnail} alt="YouTube Thumbnail" className="w-12 h-12 object-cover rounded"/>
                        )}
                    </div>
