...
export default function ProposalsView({onBack, managementId}) {
    ...
    const handleEvaluationSubmit = async (groupId, part, score) => {
        ...
        try {
            ...
            if (response.success) {
                ...
                toast({
                    title: "Error", description: "Hubo un problema al enviar la evaluación.", variant: "destructive",
                });
            }
        } catch (error) {
            console.error("Error submitting evaluation:", error);
            toast({
                title: "Error", description: "Hubo un error al enviar la evaluación.", variant: "destructive",
            });
        }
    };
    ...
    return (<div className="bg-white overflow-auto p-4 sm:p-6">
        ...
        {groups.map((group) => (<div key={group.group_id} className="mb-4 border border-purple-200 rounded-md">
            ...
            <div className="p-4 space-y-4">
                <div className="flex flex-col space-y-2">
                    ...
                    <span
                        className="text-purple-600">{group.representative.name} {group.representative.last_name}</span>
                ...
                <div className="flex justify-between items-center">
                    ...
                    {group.part_a.status === 'submitted' && group.part_a.score !== undefined ? (
                        <div className="flex items-center space-x-4">
                                    <span
                                        className="text-lg font-semibold text-green-500">Puntaje: {group.part_a.score}</span>
                        </div>) : group.part_a.status === 'submitted' ? (<div className="flex items-center space-x-4">
                        <input
                            type="number"
                            min="0"
                            max="100"
                ...
                <div className="flex justify-between items-center">
                    ...
                    {group.part_b.status === 'submitted' && group.part_b.score !== undefined ? (
                        <div className="flex items-center space-x-4">
                                    <span
                                        className="text-lg font-semibold text-green-500">Puntaje: {group.part_b.score}</span>
                        </div>) : group.part_b.status === 'submitted' ? (<div className="flex items-center space-x-4">
                        <input
                            type="number"
                            min="0"
                            max="100"
...
ProposalsView.propTypes = {
    onBack: PropTypes.func.isRequired, managementId: PropTypes.string.isRequired,
};