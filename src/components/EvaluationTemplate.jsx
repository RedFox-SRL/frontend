...
export default function EvaluationTemplate({ groupId }) {
  ...
  const fetchEvaluationTemplate = async (sprintId) => {
    ...
    try {
      const response = await getData(
        `/sprints/${sprintId}/evaluation-template`,
      );
  ...
  const handleEvaluationChange = (taskId, field, value) => {
    if (field === "comment" && value.length > 200) {
      setCommentWarnings((prev) => ({
        ...prev,
        [taskId]: "Has alcanzado el máximo de 200 caracteres",
      }));
  ...
  const handleSubmitEvaluation = async (taskId) => {
    ...
    try {
      ...
      if (!evaluation?.grade || !evaluation?.comment) {
        setToast({
          message:
            "Por favor ingresa calificación y comentario para esta tarea.",
          type: "error",
        });
        setIsSubmitting((prev) => ({
          ...prev,
      ...
      setToast({
        message: "No se pudo guardar la evaluación. Intente de nuevo.",
        type: "error",
      });
    } finally {
      setIsSubmitting((prev) => ({
  ...
  const getMemberTaskCount = (name) => {
    return tasks.filter((task) =>
      task.assigned_to.some(
        (assignee) =>
          `${assignee.user?.name} ${assignee.user?.last_name}` === name,
      ),
    ).length;
  };
    ...
    <div className="w-full">
      ...
      <div className="flex flex-col lg:flex-row items-start space-y-6 lg:space-y-0 lg:space-x-6">
        ...
        <div className="lg:w-3/4 w-full">
                ...
                >
                  ...
                  <p className="text-sm text-purple-500 mb-4">
                    ...
                    {task.assigned_to
                      .map(
                        (assignee) =>
                          `${assignee.user?.name || "N/A"} ${assignee.user?.last_name || ""}`,
                      )
                      .join(", ")}
                  </p>
          ...
          {toast && (
            <div
              className={`fixed bottom-4 right-4 p-4 rounded-md text-white ${toast.type === "error" ? "bg-red-500" : "bg-green-500"} shadow-lg transition-all duration-500 ease-in-out transform translate-y-0 opacity-100`}
            >
              {toast.message}
            </div>
          )}
