...
import { ArrowLeft, ChevronDown, ChevronUp, Users, HelpCircle } from 'lucide-react'; // Importa el ícono de ayuda
...
const RatingView2 = ({ onBack, managementId }) => {
    ...
    const [isHelpDialogOpen, setIsHelpDialogOpen] = useState(false); // Estado para el diálogo de ayuda
            ...
            {loading ? (
                ...
                <>
                    ...
                    <Dialog open={isHelpDialogOpen} onOpenChange={closeHelpDialog}>
                        <DialogContent className="bg-white p-4 sm:p-6 rounded-lg w-[95vw] sm:w-[90vw] max-w-2xl mx-auto">
                            <h4 className="text-purple-700 font-bold text-lg sm:text-xl mb-4">Ayuda</h4>
                            <p className="mb-4">Las notas de autoevaluación y pares están en "Ver detalles" con el nombre de "Auto" y "Pares".</p>
                            <p>La nota final está en base a la configuración de porcentajes establecido al crear la gestión.</p>
                        </DialogContent>
                    </Dialog>
                </>
            )}
