        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div>
            ...
            <div className="flex space-x-1 mt-1">
              {Array.from({ length: 5 }, (_, i) => (
                  <Star key={i} fill={i < Math.round(averageRating) ? "gold" : "none"} stroke="gold" />
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <ResponsiveContainer width="100%" height={250}>
              <PieChart>
                <Pie data={taskData} innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value" label={({ name, value }) => `${name}: ${value}`}>
              ...
              {taskData.map((item, index) => (
                  <div key={index} className="flex justify-between items-center">
                    ...
                    <div className="flex gap-4">
                      ...
                      <span>({((item.value / totalTasksCount) * 100).toFixed(1)}%)</span> {/* Porcentaje de participación */}
                    </div>
                  </div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredTasks.map(task => (
                <div key={task.id} className="bg-white p-4 rounded-lg shadow-md">
                  ...
                  {task.links.length > 0 && (
                      <div className="mt-2">
                        <a href={task.links[0].url} target="_blank" rel="noopener noreferrer" className="text-blue-500 underline">
                          {task.links[0].description}
                        </a>
                      </div>
                  )}
                  <div className="mt-2">
                    <p className="font-semibold">Satisfacción: <span className="text-purple-600">{["Malo", "Regular", "Aceptable", "Bueno", "Excelente"][task.satisfaction_level - 1]}</span></p>
